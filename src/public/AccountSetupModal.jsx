...
/**
 * AccountSetupModal
 * Blocks the entire app on first login until the player
 * enters a display name (and optional phone number).
 * Dismissed automatically once profile_setup = true.
 */
export function AccountSetupModal() {
  ...
  const handleSubmit = async (e) => {
    ...
    // profile_setup is now true → needsSetup false → modal unmounts
  }

  return (
    <div
      className="fixed inset-0 z-[9999] flex items-center justify-center bg-slate-950/90 backdrop-blur-sm px-4"
      aria-modal="true" role="dialog" aria-labelledby="setup-title"
    >
      <div className="w-full max-w-sm rounded-2xl border border-slate-700 bg-slate-900 shadow-2xl shadow-slate-950/80 p-6 space-y-6">
        ...
        <div className="space-y-3">
          ...
          <div>
            ...
            <p className="mt-1 text-xs text-slate-400">
              Signed in as <span className="text-slate-300">{user?.email}</span>. Pick a display name to continue.
            </p>
          </div>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          {status === 'name-error' && <p className="text-[11px] text-red-400">❌ Name contains invalid characters.</p>}
