    ...
    <div className="flex min-h-screen items-center justify-center bg-slate-950 px-4">
      <div className="w-full max-w-sm rounded-2xl border border-slate-800 bg-slate-900/80 p-6 shadow-xl">
        <div className="mb-4 space-y-1 text-center">
          <p className="text-xs font-semibold uppercase tracking-[0.2em] text-slate-400">Tournvia</p>
        ...
        <form onSubmit={handleSubmit} className="space-y-3 text-xs text-slate-200">
          ...
          {status === 'error' && (
            <p className="text-[11px] text-red-400">Login failed. Check credentials inserted in Supabase auth.</p>
          )}
        </form>
      </div>
    </div>
